...
<template>
    ...
    <div class="container mt-5" v-if="permisos.includes('access.edit.seo')"> 
        ...
        <div class="d-flex justify-content-start">
            <div class="ml-3 mb-5" v-bind:style="{ 
                                ...
                                }"><span v-bind:style="{ color: ds_admin[2].linea }">E</span>ditar SEO</div>
            ...
            <div class="col-12 col-sm-12 col-md-12 d-flex align-items-stretch">
              <div class="card" :style="{ backgroundColor: ds_admin[4].backgroundColor }">
                <div class="card-header border-bottom-0  d-flex justify-content-end">
                  <span :style="{ color:'white' }">Última modificación:  {{ fecha_formateada }}</span>
                </div>
                <div class="card-body pt-0">
                    ...
                    <form class="text-left p-3" @submit.prevent="submit" v-bind:style="{ 
                        ...
                        <div class="alert alert-info alert-dismissible mt-5">
                            ...
                            <h5 class="d-flex align-items-center subrayar">
                        ...
                        <div class="md-form mt-3" v-if="title != null">
                            <input type="text" id="idtitle" class="form-control" v-model="title">
                            <div class="d-flex justify-content-end" v-if="title.length >= 0 ? contadorCaracteresTitle() : ''">
                                <p class="marco mr-2" :style="{ color: contadorTitle > 60 ? 'red' : 'white' }"> Caracteres: {{ contadorTitle }} {{ }}</p>
                        ...
                        <ckeditor :editor="editor" v-model="description" :config="editorConfig" tag-name="textarea" class="mt-5 text-dark"></ckeditor>
    ...
    export default {
        ...
        methods:{
            ...
            getDatos(){
                ...
                let url = '/api/seo/'+this.idrecibido;
                axios.get(url).then(response => {
                    
                    this.pagina =response.data.data[0].pagina;
                    this.titulo =response.data.data[0].titulo;
                    if(response.data.data[0].title != null){this.title = response.data.data[0].title;}else{this.title = '';}
                    if(response.data.data[0].description != null){this.description = response.data.data[0].description;}else{this.description = '';}
                    this.autor =response.data.data[0].autor;
                    this.h1 =response.data.data[0].h1;
                    this.h2 =response.data.data[0].h2;
                    this.canonical =response.data.data[0].extra;
                    this.fecha_formateada =response.data.data[0].fecha_formateada;
